...
public class EnablePlatformAdministratorHandlerTests(IdentityDatabaseFixture fixture)
{
    private static readonly DateTimeOffset Now = new(2026, 6, 6, 12, 0, 0, TimeSpan.Zero);
    private static readonly FakeClock Clock = new(Now.UtcDateTime);

    private EnablePlatformAdministratorHandler BuildHandler(FakeAuditEventPublisher auditPublisher) =>
    ...
    private async Task<Guid> SeedTargetAsync(bool isEnabled = false)
    {
        await using var db = fixture.BuildContext();
    ...
    public async Task HandleAsync_Returns_Unauthorized_When_Caller_Is_Not_A_PlatformOwner()
    {
        ...
        var result = await handler.HandleAsync(
            new EnablePlatformAdministratorRequest(targetId), currentUser, CancellationToken.None);
    ...
    public async Task HandleAsync_Returns_NotFound_When_Administrator_Missing()
    {
        ...
        var result = await handler.HandleAsync(
            new EnablePlatformAdministratorRequest(Guid.NewGuid()), currentUser, CancellationToken.None);
    ...
    [Fact]
    public async Task HandleAsync_Returns_Conflict_When_Already_Enabled()
    {
        var ownerEmail = await SeedOwnerAsync();
        var targetId = await SeedTargetAsync(isEnabled: true);
        var handler = BuildHandler(new FakeAuditEventPublisher());
        var currentUser = new FakeCurrentUser(Guid.NewGuid(), ownerEmail);
    ...
    [Fact]
    public async Task HandleAsync_Enables_Administrator_And_Publishes_Audit_Event_On_Happy_Path()
    {
        var ownerEmail = await SeedOwnerAsync();
        ...
        Assert.Single(auditPublisher.PublishedEvents, e => e is PlatformAdministratorEnabledAuditEvent);
    }
}
