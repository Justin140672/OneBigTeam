...
public class CancelCustomerDeletionHandlerTests
{
    ...
    public async Task HandleAsync_Returns_Unauthorized_When_Email_Not_On_AllowList()
    {
        ...
        var result = await handler.HandleAsync(
            new CancelCustomerDeletionRequest { CompanyId = companyId, Reason = "Customer reconsidered" },
    ...
    public async Task HandleAsync_Returns_NotFound_When_No_Subscription_Row_Exists()
    {
        ...
        var result = await handler.HandleAsync(
            new CancelCustomerDeletionRequest { CompanyId = Guid.NewGuid(), Reason = "Customer reconsidered" },
            CancellationToken.None);
    ...
    [Fact]
    public async Task HandleAsync_Cancels_Deletion_Persists_And_Publishes_Audit_Event_On_Success()
    {
        await using var context = BuildContext();
        var companyId = Guid.NewGuid();
        var subscription = CustomerSubscription.StartTrial(companyId, Now, trialLengthDays: 14);
        ...
        Assert.Equal(Now, result.Value!.DeletionCancelledAt);

        var persisted = await context.CustomerSubscriptions.SingleAsync(s => s.CompanyId == companyId);
        Assert.Equal(Now, persisted.DeletionCancelledAt);
        Assert.False(persisted.HasPendingDeletion);

        var published = Assert.Single(publisher.Published);
        var auditEvent = Assert.IsType<CustomerDeletionCancelledAuditEvent>(published);
        Assert.Equal(companyId, auditEvent.CompanyId);
        Assert.Equal(actorId, auditEvent.ActorUserId);
        Assert.Equal("Customer reconsidered", auditEvent.Reason);
    }

    private static CancelCustomerDeletionHandler BuildHandler(
    ...
    {
        return new CancelCustomerDeletionHandler(
            context,
            currentUser,
            configuration,
