...
public class ExtendCustomerTrialHandlerTests
{
    ...
    public async Task HandleAsync_Returns_Unauthorized_When_Email_Not_On_AllowList()
    {
        ...
        var result = await handler.HandleAsync(
            new ExtendCustomerTrialRequest { CompanyId = companyId, NewTrialExpiresAt = Now.AddDays(30), Reason = "Extending trial" },
    ...
    public async Task HandleAsync_Returns_NotFound_When_No_Subscription_Row_Exists()
    {
        ...
        var result = await handler.HandleAsync(
            new ExtendCustomerTrialRequest { CompanyId = Guid.NewGuid(), NewTrialExpiresAt = Now.AddDays(30), Reason = "Extending trial" },
            CancellationToken.None);
    ...
    [Fact]
    public async Task HandleAsync_Returns_Validation_Failure_When_Subscription_Is_Active()
    {
        await using var context = BuildContext();
        var companyId = Guid.NewGuid();
        var subscription = CustomerSubscription.StartTrial(companyId, Now, trialLengthDays: 14);
    ...
    [Fact]
    public async Task HandleAsync_Extends_Trial_Persists_And_Publishes_Audit_Event_On_Success()
    {
        await using var context = BuildContext();
        var companyId = Guid.NewGuid();
        var subscription = CustomerSubscription.StartTrial(companyId, Now, trialLengthDays: 14);
        ...
        var result = await handler.HandleAsync(
            new ExtendCustomerTrialRequest { CompanyId = companyId, NewTrialExpiresAt = newExpiry, Reason = "Extending trial for pilot" },
            CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(companyId, result.Value!.CompanyId);
        Assert.Equal(newExpiry, result.Value.TrialExpiresAt);
        Assert.Equal(SubscriptionStatus.Trial.ToString(), result.Value.Status);

        var persisted = await context.CustomerSubscriptions.SingleAsync(s => s.CompanyId == companyId);
        Assert.Equal(newExpiry, persisted.TrialExpiresAt);

        var published = Assert.Single(publisher.Published);
        var auditEvent = Assert.IsType<TrialExtendedAuditEvent>(published);
        Assert.Equal(companyId, auditEvent.CompanyId);
        Assert.Equal(actorId, auditEvent.ActorUserId);
        Assert.Equal("Extending trial for pilot", auditEvent.Reason);
    }

    private static ExtendCustomerTrialHandler BuildHandler(
    ...
    {
        return new ExtendCustomerTrialHandler(
            context,
            currentUser,
            configuration,
