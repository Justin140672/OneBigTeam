...
public class GetMySicknessRecordsHandlerTests
{
    ...
    private static async Task<SicknessRecord> SeedRecord(
        SicknessDbContext db, Guid companyId, Guid employeeId, Guid categoryId, DateOnly startDate,
        SicknessEvidenceStatus evidenceStatus = SicknessEvidenceStatus.NotRequired)
    {
        var now = new DateTimeOffset(FixedUtcNow, TimeSpan.Zero);
    ...
    public async Task HandleAsync_Returns_Empty_List_When_No_Records()
    {
        ...
        var handler = new GetMySicknessRecordsHandler(db);
        var result = await handler.HandleAsync(new GetMySicknessRecordsRequest
        {
    ...
    [Fact]
    public async Task HandleAsync_Returns_Own_Records_Ordered_By_StartDate_Descending()
    {
        await using var db = BuildContext();
