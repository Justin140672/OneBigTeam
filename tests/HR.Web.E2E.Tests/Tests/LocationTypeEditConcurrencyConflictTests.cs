...
/// <summary>
/// Ticket 2: optimistic-concurrency conflict UI on the Location Type edit page
/// (LocationTypeEdit.razor, /companies/{companyId}/location-types/{id}), which renders the shared
/// &lt;SaveConflictBanner&gt; via EditPageBase. When a save is rejected with HTTP 409 because the
/// location type's Version moved on since the form loaded it, the page renders the banner ("Someone
/// else changed this location type while you were editing. Your changes have not been saved.") plus
...
/// same location type and saves first, bumping its Version. Each test creates its own uniquely-named
/// location type, so nothing here contends with seeded data or the other parallel test files —
...
/// </summary>
public sealed class LocationTypeEditConcurrencyConflictTests(HrAdminPersonaFixture fixture)
    : RoleE2ETestBase<HrAdminPersonaFixture>(fixture)
{
    ...
    [Fact]
    public async Task LocationTypeEdit_PageLoads_ShowsNameInForm()
    {
        var login = new LoginPage(_page, _fixture.WebBaseUrl);
        var list  = new LocationTypeListPage(_page, _fixture.WebBaseUrl);
        var edit  = new LocationTypeEditPage(_page, _fixture.WebBaseUrl);

        await login.GoToAsync();
        await login.LoginAsync(LauraEmail);

        var (name, id) = await CreateLocationTypeAsync(list, edit);
    ...
    [Fact]
    public async Task LocationTypeEdit_NormalEditAndSave_PersistsChange()
    {
        var login = new LoginPage(_page, _fixture.WebBaseUrl);
        var list  = new LocationTypeListPage(_page, _fixture.WebBaseUrl);
        var edit  = new LocationTypeEditPage(_page, _fixture.WebBaseUrl);
        ...
        var (_, id) = await CreateLocationTypeAsync(list, edit);
    ...
    [Fact]
    public async Task LocationTypeEdit_SaveAfterAnotherActorChanged_ShowsConflictBanner_ThenReloadRecovers()
    {
        var login = new LoginPage(_page, _fixture.WebBaseUrl);
        var list  = new LocationTypeListPage(_page, _fixture.WebBaseUrl);
        var edit  = new LocationTypeEditPage(_page, _fixture.WebBaseUrl);
        ...
        try
        {
            var otherEdit = new LocationTypeEditPage(otherPage, _fixture.WebBaseUrl);
    ...
    private async Task<(string Name, Guid Id)> CreateLocationTypeAsync(LocationTypeListPage list, LocationTypeEditPage edit)
    {
        var name = $"E2E LTConflict {Guid.NewGuid().ToString("N")[..8]}";
        ...
        Assert.True(await list.HasItemAsync(name), $"Failed to seed location type '{name}'");
