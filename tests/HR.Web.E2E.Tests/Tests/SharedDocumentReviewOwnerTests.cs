...
public sealed class SharedDocumentReviewOwnerTests(AppFixture fixture) : E2ETestBase(fixture)
{
    ...
    // Review Owner before submitting via the dialog's filterable employee picker. Category is the
    // first combobox in the dialog; Review Owner is reached by scoping to its own ".col-md-6"
    // field group (rather than by combobox index) since it sits after the conditional "Custom
    // Frequency (months)" field, whose presence would otherwise shift a plain Nth() index.
