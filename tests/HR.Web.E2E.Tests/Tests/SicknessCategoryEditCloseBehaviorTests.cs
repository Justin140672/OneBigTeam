...
/// </summary>
[Collection("E2E")]
public sealed class SicknessCategoryEditCloseBehaviorTests(AppFixture fixture) : E2ETestBase(fixture)
{
    ...
    public async Task Close_ExistingRecordWithNoChanges_NavigatesDirectlyToList()
    {
        ...
        Assert.EndsWith("/sickness-categories", _page.Url);
    }
    ...
    public async Task Close_NewRecordWithUnsavedChanges_ShowsConfirmDialog()
    {
        ...
        await catEdit.FillNameAsync("Unsaved Sickness Category");

        await catEdit.ClickCloseAsync();

        Assert.True(await catEdit.IsUnsavedChangesDialogVisibleAsync());
        Assert.Contains("/sickness-categories/new", _page.Url);
    }
