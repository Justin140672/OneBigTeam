...
/// <summary>
/// Direct coverage of the Close / unsaved-changes prompt (EditPageBase) on the Document Type
/// edit page.
/// </summary>
public sealed class DocumentTypeEditCloseBehaviorTests(HrAdminPersonaFixture fixture) : RoleE2ETestBase<HrAdminPersonaFixture>(fixture)
{
    ...
    public async Task Close_ExistingRecordWithNoChanges_NavigatesDirectlyToList()
    {
        ...
        Assert.EndsWith("/document-types", _page.Url);
    }
    ...
    public async Task Close_NewRecordWithUnsavedChanges_ShowsConfirmDialog()
    {
        ...
        await typeEdit.FillNameAsync("Unsaved Document Type");

        await typeEdit.ClickCloseAsync();

        Assert.True(await typeEdit.IsUnsavedChangesDialogVisibleAsync());
        Assert.Contains("/document-types/new", _page.Url);
    }
