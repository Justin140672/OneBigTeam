...
public sealed class RecruitmentPipelineReportPage(IPage page, string baseUrl)
{
    ...
    {
        await page.GotoAsync($"{baseUrl}/companies/{companyId}/reporting/recruitment-pipeline");
    ...
    public async Task SelectGroupByAsync(string groupByText)
    {
        await DropDownSelector.SelectAsync(page, GroupByField, groupByText);
        await page.WaitForSelectorAsync(RowsRenderedSelector, new() { Timeout = 15_000 });
    }
