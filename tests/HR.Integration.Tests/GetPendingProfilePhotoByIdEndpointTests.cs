...
public class GetPendingProfilePhotoByIdEndpointTests : IClassFixture<ApiWebApplicationFactory>
{
    private readonly ApiWebApplicationFactory _factory;
    private static readonly Guid ManagerUser = Guid.Parse("ff100001-0000-0000-0000-000000000001");

    public GetPendingProfilePhotoByIdEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
        Task.Run(async () =>
        {
    ...
    public async Task Get_Returns_Unauthorized_For_Anonymous_Request()
    {
        ...
        var response = await client.GetAsync(
            $"/api/companies/{Guid.NewGuid()}/profile-photo/pending/{Guid.NewGuid()}");
    ...
    public async Task Get_Returns_NotFound_When_PendingPhotoId_Belongs_To_Different_Company()
    {
        ...
        using (var managerClientB = ManagerClient(companyB))
        {
            ...
            var payload = await lookup.Content.ReadFromJsonAsync<PendingProfilePhotoPayload>();
    ...
    public async Task Get_Returns_Ok_With_Populated_Response()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, lookup.StatusCode);
        var lookupPayload = await lookup.Content.ReadFromJsonAsync<PendingProfilePhotoPayload>();

        var response = await client.GetAsync(
            $"/api/companies/{companyId}/profile-photo/pending/{lookupPayload!.Id}");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        var payload = await response.Content.ReadFromJsonAsync<PendingProfilePhotoPayload>();
        Assert.NotNull(payload);
        Assert.Equal(lookupPayload.Id, payload!.Id);
