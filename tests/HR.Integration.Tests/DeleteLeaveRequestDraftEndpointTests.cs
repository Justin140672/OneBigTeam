...
public class DeleteLeaveRequestDraftEndpointTests
{
    private readonly ApiWebApplicationFactory _factory;

    private static readonly Guid User1 = new("cccccccc-0000-0000-0000-000000000004");
    ...
    public DeleteLeaveRequestDraftEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    private async Task<(HttpClient Client, Guid EmployeeId)> SetupEmployeeAsync()
    {
        ...
        var employeeResponse = await client.PostAsJsonAsync(
            ...
            new
            {
                ...
                firstName = "Draft",
                lastName = "Deleter",
                workEmail = $"draft.deleter.{Guid.NewGuid():N}@example.com",
                startDate = "2026-01-01",
                dateOfBirth = "1990-01-01",
                nationality = "British",
                gender = "Male",
                employeeNumber = $"DD-{Guid.NewGuid():N}",
    ...
    [Fact]
    public async Task Delete_Draft_Returns_Ok()
    {
    ...
    [Fact]
    public async Task Delete_Draft_Returns_Unauthorized_For_Anonymous_Request()
    {
    ...
    [Fact]
    public async Task Delete_Draft_Returns_NotFound_For_Unknown_Id()
    {
        var (client, employeeId) = await SetupEmployeeAsync();
        var companyId = SeededCompanyId;
    ...
    [Fact]
    public async Task Delete_Draft_Returns_BadRequest_When_Not_A_Draft()
    {
        var (client, employeeId) = await SetupEmployeeAsync();
        var companyId = SeededCompanyId;
