...
public class AdminCancelSubscriptionEndpointTests
{
    ...
    public AdminCancelSubscriptionEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    private async Task<Guid> SeedCanceledSubscriptionAsync(DateTimeOffset now)
    {
        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<CompaniesDbContext>();
    ...
    private string Url(Guid companyId) => $"/api/companies/admin/customers/{companyId}/subscription/cancel";

    [Fact]
    public async Task Post_CancelSubscription_Returns_Unauthorized_For_Anonymous_Request()
    {
    ...
    [Fact]
    public async Task Post_CancelSubscription_Returns_Unauthorized_For_Authenticated_Caller_Not_On_AllowList()
    {
    ...
    [Fact]
    public async Task Post_CancelSubscription_Returns_NotFound_For_Unknown_Company()
    {
    ...
    [Fact]
    public async Task Post_CancelSubscription_Returns_UnprocessableEntity_When_Reason_Is_Missing()
    {
    ...
    [Fact]
    public async Task Post_CancelSubscription_Returns_BadRequest_When_Already_Canceled()
    {
        var companyId = await SeedCanceledSubscriptionAsync(DateTimeOffset.UtcNow);
        using var client = ClientFor(Guid.NewGuid(), AllowListedEmail);

        var response = await client.PostAsJsonAsync(
            Url(companyId), new { reason = "Customer requested cancellation via support call" });

        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }

    [Fact]
    public async Task Post_CancelSubscription_Returns_Ok_And_Schedules_Cancellation_For_AllowListed_Caller()
    {
        var now = DateTimeOffset.UtcNow;
        var companyId = await SeedActiveSubscriptionAsync(now);
        ...
        var payload = await response.Content.ReadFromJsonAsync<CancelSubscriptionPayload>();
        Assert.NotNull(payload);
        Assert.Equal(companyId, payload!.CompanyId);
        Assert.True(payload.CancelAtPeriodEnd);
        ...
        var auditRecord = await auditDb.AuditEvents
            .Where(e => e.CompanyId == companyId && e.EventType == "subscription.admin-cancelled")
    ...
    private sealed record CancelSubscriptionPayload(Guid CompanyId, bool CancelAtPeriodEnd);
}
