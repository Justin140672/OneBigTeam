...
public class ListLocationTypesEndpointTests : IClassFixture<ApiWebApplicationFactory>
{
    private readonly ApiWebApplicationFactory _factory;
    private static readonly Guid AdminUserId = new("ff000004-0000-0000-0000-000000000001");

    public ListLocationTypesEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Get_LocationTypes_Returns_Unauthorized_For_Anonymous_Request()
    {
        using var client = _factory.CreateClient();

        var response = await client.GetAsync($"/api/companies/{Guid.NewGuid()}/location-types");
    ...
    public async Task Get_LocationTypes_Returns_Created_Types()
    {
        ...
        Assert.Equal("Office", payload.Items[0].Name);
        Assert.Equal("Warehouse", payload.Items[1].Name);
        Assert.All(payload.Items, item => Assert.True(item.IsActive));
    }

    [Fact]
    public async Task Get_LocationTypes_Scopes_To_Company()
    {
    ...
    private sealed record ListPayload(IReadOnlyList<LocationTypeItem> Items);

    private sealed record LocationTypeItem(
        Guid Id,
        Guid CompanyId,
        string Name,
        string? Description,
