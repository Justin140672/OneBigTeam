...
public class ResumeCustomerServiceEndpointTests
{
    private const string AllowListedEmail = "priya.shah@acme.example";

    private readonly ApiWebApplicationFactory _factory;

    public ResumeCustomerServiceEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    private string Url(Guid companyId) => $"/api/companies/admin/customers/{companyId}/subscription/resume-service";

    [Fact]
    public async Task Post_ResumeService_Returns_Unauthorized_For_Anonymous_Request()
    {
        using var client = _factory.CreateClient();

        var response = await client.PostAsJsonAsync(
            Url(Guid.NewGuid()), new { reason = "Investigation concluded, resuming service" });

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task Post_ResumeService_Returns_Unauthorized_For_Authenticated_Caller_Not_On_AllowList()
    {
    ...
    [Fact]
    public async Task Post_ResumeService_Returns_NotFound_For_Unknown_Company()
    {
    ...
    [Fact]
    public async Task Post_ResumeService_Returns_UnprocessableEntity_When_Reason_Is_Too_Short()
    {
    ...
    [Fact]
    public async Task Post_ResumeService_Returns_BadRequest_When_Not_Currently_Forced()
    {
        var companyId = await SeedTrialSubscriptionAsync(DateTimeOffset.UtcNow);
        using var client = ClientFor(Guid.NewGuid(), AllowListedEmail);

        var response = await client.PostAsJsonAsync(
            Url(companyId), new { reason = "Investigation concluded, resuming service" });

        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }

    [Fact]
    public async Task Post_ResumeService_Returns_Ok_And_Resumes_Service_For_AllowListed_Caller()
    {
        var now = DateTimeOffset.UtcNow;
        var companyId = await SeedForcedReadOnlySubscriptionAsync(now);
        ...
        var payload = await response.Content.ReadFromJsonAsync<ResumeServicePayload>();
        ...
        var auditRecord = await auditDb.AuditEvents
            .Where(e => e.CompanyId == companyId && e.EventType == "subscription.admin-resumed-service")
    ...
    private sealed record ResumeServicePayload(Guid CompanyId, bool AdminForcedReadOnly);
}
