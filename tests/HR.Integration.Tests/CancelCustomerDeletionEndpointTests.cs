...
public class CancelCustomerDeletionEndpointTests
{
    private const string AllowListedEmail = "priya.shah@acme.example";

    private readonly ApiWebApplicationFactory _factory;

    public CancelCustomerDeletionEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    private string Url(Guid companyId) => $"/api/companies/admin/customers/{companyId}/subscription/cancel-deletion";

    [Fact]
    public async Task Post_CancelDeletion_Returns_Unauthorized_For_Anonymous_Request()
    {
        using var client = _factory.CreateClient();

        var response = await client.PostAsJsonAsync(
            Url(Guid.NewGuid()), new { reason = "Customer reconsidered account closure" });

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task Post_CancelDeletion_Returns_Unauthorized_For_Authenticated_Caller_Not_On_AllowList()
    {
    ...
    [Fact]
    public async Task Post_CancelDeletion_Returns_NotFound_For_Unknown_Company()
    {
    ...
    [Fact]
    public async Task Post_CancelDeletion_Returns_UnprocessableEntity_When_Reason_Is_Too_Short()
    {
    ...
    [Fact]
    public async Task Post_CancelDeletion_Returns_BadRequest_When_No_Pending_Deletion()
    {
        var companyId = await SeedTrialSubscriptionAsync(DateTimeOffset.UtcNow);
        using var client = ClientFor(Guid.NewGuid(), AllowListedEmail);

        var response = await client.PostAsJsonAsync(
            Url(companyId), new { reason = "Customer reconsidered account closure" });

        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }

    [Fact]
    public async Task Post_CancelDeletion_Returns_Ok_And_Cancels_Deletion_For_AllowListed_Caller()
    {
        ...
        var payload = await response.Content.ReadFromJsonAsync<CancelDeletionPayload>();
        ...
        Assert.NotNull(persisted.DeletionCancelledAt);
        Assert.False(persisted.HasPendingDeletion);

        var auditDb = scope.ServiceProvider.GetRequiredService<AuditDbContext>();
        var auditRecord = await auditDb.AuditEvents
            .Where(e => e.CompanyId == companyId && e.EventType == "subscription.deletion-cancelled")
    ...
    private sealed record CancelDeletionPayload(Guid CompanyId, DateTimeOffset DeletionCancelledAt);
}
