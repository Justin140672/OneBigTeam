...
public class DisablePlatformAdministratorEndpointTests
{
    private readonly ApiWebApplicationFactory _factory;

    public DisablePlatformAdministratorEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    [Fact]
    public async Task Post_DisablePlatformAdministrator_Returns_Unauthorized_For_Anonymous_Request()
    {
        using var client = _factory.CreateClient();

        var response = await client.PostAsync(
            $"/api/platform-administrators/{Guid.NewGuid()}/disable", EmptyJson());

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task Post_DisablePlatformAdministrator_Returns_Unauthorized_When_Caller_Is_Not_A_PlatformOwner()
    {
    ...
    [Fact]
    public async Task Post_DisablePlatformAdministrator_Returns_NotFound_When_Administrator_Missing()
    {
    ...
    [Fact]
    public async Task Post_DisablePlatformAdministrator_Returns_Conflict_When_Already_Disabled()
    {
        var (_, ownerEmail) = await PlatformAdministratorTestHelpers.SeedAdministratorAsync(
            _factory, PlatformAdministratorRole.PlatformOwner);
        ...
        var response = await client.PostAsync($"/api/platform-administrators/{targetId}/disable", EmptyJson());

        Assert.Equal(HttpStatusCode.Conflict, response.StatusCode);
    }

    [Fact]
    public async Task Post_DisablePlatformAdministrator_Disables_Administrator_On_Happy_Path()
    {
        var (_, ownerEmail) = await PlatformAdministratorTestHelpers.SeedAdministratorAsync(
            _factory, PlatformAdministratorRole.PlatformOwner);
        var (targetId, _) = await PlatformAdministratorTestHelpers.SeedAdministratorAsync(
