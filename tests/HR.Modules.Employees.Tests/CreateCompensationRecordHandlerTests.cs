...
public class CreateCompensationRecordHandlerTests
{
    ...
    public async Task HandleAsync_Returns_NotFound_When_Employee_Does_Not_Exist()
    {
        ...
        var handler = new CreateCompensationRecordHandler(context, new FakeClock(FixedUtcNow), publisher);
    ...
    public async Task HandleAsync_Closes_Previous_Open_Record_When_Creating_New_One()
    {
        ...
        var existing = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2025, 1, 1), SalaryType.Annual, 40000m, "GBP", 37.5m, 1m, null, now);
    ...
    public async Task HandleAsync_Returns_Conflict_When_EffectiveFrom_Not_After_Open_Record()
    {
        ...
        var existing = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2026, 1, 1), SalaryType.Annual, 40000m, "GBP", null, null, null, now);
        context.Compensations.Add(existing);
        await context.SaveChangesAsync();
    ...
    public async Task HandleAsync_Returns_Conflict_When_EffectiveFrom_Falls_Inside_Closed_Historical_Record()
    {
        ...
        var historical = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2024, 1, 1), SalaryType.Annual, 35000m, "GBP", null, null, null, now);
        historical.Close(new DateOnly(2024, 12, 31), now);
        var open = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2025, 1, 1), SalaryType.Annual, 40000m, "GBP", null, null, null, now);
    ...
    public async Task HandleAsync_Returns_Conflict_When_New_Open_Ended_Record_Would_Swallow_A_Future_Historical_Record()
    {
        ...
        var futureHistorical = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2027, 1, 1), SalaryType.Annual, 50000m, "GBP", null, null, null, now);
    ...
    public async Task HandleAsync_Does_Not_Close_Already_Closed_Records()
    {
        ...
        var alreadyClosed = Compensation.Create(Guid.NewGuid(), companyId, employee.Id, new DateOnly(2024, 1, 1), SalaryType.Annual, 30000m, "GBP", null, null, null, now);
