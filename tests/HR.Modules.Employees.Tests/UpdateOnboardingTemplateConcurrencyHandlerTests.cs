...
// Ticket 2 (optimistic concurrency rollout): OnboardingTemplate.Version coverage for the
// UpdateOnboardingTemplate slice.
public class UpdateOnboardingTemplateConcurrencyHandlerTests
{
    ...
    private static UpdateOnboardingTemplateHandler BuildHandler(EmployeesDbContext ctx)
    ...
    private static async Task<(string DbName, Guid CompanyId, Guid Id)> SeedAsync()
    {
        ...
        var template = OnboardingTemplate.Create(Guid.NewGuid(), companyId, "Original", null, now);
        seed.OnboardingTemplates.Add(template);
        await seed.SaveChangesAsync();

        return (dbName, companyId, template.Id);
    }

    private static UpdateOnboardingTemplateRequest Request(Guid companyId, Guid id, int? expectedVersion, string name)
        => new() { CompanyId = companyId, Id = id, Name = name, Tasks = [], ExpectedVersion = expectedVersion };
    ...
    public async Task Matching_ExpectedVersion_Succeeds_And_Bumps_Version()
    {
        ...
        Assert.Equal(2, (await verify.OnboardingTemplates.SingleAsync()).Version);
    }
    ...
    public async Task Stale_ExpectedVersion_Returns_Concurrency_Failure_And_Preserves_Committed_Values()
    {
        ...
        await ctxA.OnboardingTemplates.SingleAsync();
        ...
        await using var verify = new EmployeesDbContext(Options(dbName));
        var saved = await verify.OnboardingTemplates.SingleAsync();
