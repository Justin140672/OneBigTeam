...
using HR.Modules.Employees.Features.CreateTimelineEntryOnProbationFailed;
using HR.Modules.Employees.Tests.Infrastructure;
using HR.SharedKernel;

namespace HR.Modules.Employees.Tests;

public class ProbationFailedHandlerTests
{
    [Fact]
    public async Task HandleAsync_Writes_Timeline_Entry()
    {
        var timelineWriter = new FakeEmployeeTimelineWriter();
        var handler = new ProbationFailedHandler(timelineWriter);
        ...
        await handler.HandleAsync(
            new ProbationFailedIntegrationEvent(companyId, employeeId, probationRecordId, occurredAt),
        ...
        Assert.Equal(EmployeeTimelineEventType.ProbationFailed, entry.EventType);
    ...
    [Fact]
    public async Task HandleAsync_Summary_Text_Is_Fixed_And_Contains_No_Free_Text_Notes()
    {
        var timelineWriter = new FakeEmployeeTimelineWriter();
        var handler = new ProbationFailedHandler(timelineWriter);
        ...
        await handler.HandleAsync(
            new ProbationFailedIntegrationEvent(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), occurredAt),
            CancellationToken.None);

        var entry = Assert.Single(timelineWriter.Added);
        Assert.Equal("Probation failed", entry.Title);
        Assert.Equal("Probation period failed.", entry.Summary);
    }
}
