...
// Ticket 2 (optimistic concurrency rollout): LocationType.Version coverage for the
// UpdateLocationType slice.
public class UpdateLocationTypeConcurrencyHandlerTests
{
    ...
    private static UpdateLocationTypeHandler BuildHandler(EmployeesDbContext ctx)
    ...
    private static async Task<(string DbName, Guid CompanyId, Guid Id)> SeedAsync()
    {
        ...
        var entity = LocationType.Create(Guid.NewGuid(), companyId, "Original", null, now);
        seed.LocationTypes.Add(entity);
        await seed.SaveChangesAsync();

        return (dbName, companyId, entity.Id);
    }

    private static UpdateLocationTypeRequest Request(Guid companyId, Guid id, int? expectedVersion, string name)
    ...
    public async Task Matching_ExpectedVersion_Succeeds_And_Bumps_Version()
    {
        ...
        Assert.Equal(2, (await verify.LocationTypes.SingleAsync()).Version);
    }
    ...
    public async Task Stale_ExpectedVersion_Returns_Concurrency_Failure_And_Preserves_Committed_Values()
    {
        ...
        await ctxA.LocationTypes.SingleAsync();
        ...
        await using var verify = new EmployeesDbContext(Options(dbName));
        var saved = await verify.LocationTypes.SingleAsync();
