...
public class CreateRecruitmentStageValidatorTests
{
    private readonly CreateRecruitmentStageValidator _validator = new();

    private static CreateRecruitmentStageRequest Valid(bool isTerminal = false, RecruitmentStagePurpose? purpose = null) =>
        new(Guid.NewGuid(), "Interview", 3, isTerminal,
    ...
    public void Fails_For_Undefined_Purpose_Enum_Value()
    {
        ...
        Assert.Contains(result.Errors, e => e.PropertyName == nameof(CreateRecruitmentStageRequest.Purpose));
    }

    [Fact]
    public void Fails_When_Purpose_Set_On_Terminal_Stage()
    {
        var result = _validator.Validate(Valid(isTerminal: true, purpose: RecruitmentStagePurpose.Offer));
