...
public class RequestAssetReturnHandlerTests
{
    ...
    private static async Task<(Guid assignmentId, Guid employeeId, Guid companyId)> SeedActiveAssignmentAsync(AssetsDbContext db)
    {
        ...
        var assetResult = await new CreateAssetHandler(db, clock, new FakeCompanyAssetNumberSettingsReader(), new FakeAssetNumberGenerator()).HandleAsync(new CreateAssetRequest
        {
            CompanyId = companyId,
