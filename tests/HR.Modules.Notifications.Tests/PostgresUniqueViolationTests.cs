...
/// <summary>
/// OBT-REM-03: <see cref="PostgresUniqueViolation.Is"/> must recognise a 23505 unique-constraint
/// violation surfaced through EF Core, optionally narrowed to a named index, and must not mistake an
/// unrelated <see cref="DbUpdateException"/> for one.
/// </summary>
public class PostgresUniqueViolationTests
{
    private const string NotificationsIndex = "IX_notifications_employee_id_source_entity_id_type";
    ...
    public void Recognises_duplicate_key_message()
    {
        var ex = Wrap(new Exception(
            $"23505: duplicate key value violates unique constraint \"{NotificationsIndex}\""));

        Assert.True(PostgresUniqueViolation.Is(ex));
        Assert.True(PostgresUniqueViolation.Is(ex, NotificationsIndex));
    }

    [Fact]
    public void Recognises_via_sqlstate_property()
    {
        var ex = Wrap(new PostgresException("boom", "23505", NotificationsIndex));
    ...
    public void Constraint_narrowing_rejects_a_different_index()
    {
        ...
        Assert.True(PostgresUniqueViolation.Is(ex));
        Assert.False(PostgresUniqueViolation.Is(ex, NotificationsIndex));
    }
