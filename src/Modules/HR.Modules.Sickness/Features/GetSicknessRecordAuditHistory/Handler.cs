...
{
    private const string EntityType = "SicknessRecord";

    public async Task<Result<GetSicknessRecordAuditHistoryResponse>> HandleAsync(
        Guid companyId,
        Guid sicknessRecordId,
        CancellationToken cancellationToken)
    {
        var entries = await auditHistoryReader.GetEntityAuditHistoryAsync(
            companyId, EntityType, sicknessRecordId, cancellationToken);
        ...
        var items = entries
            .Select(e => new SicknessAuditHistoryItem(
                e.OccurredAt,
        ...
        return Result.Success(new GetSicknessRecordAuditHistoryResponse(items));
    }
    ...
    private static IReadOnlyList<SicknessAuditFieldChangeItem> BuildChanges(
        ...
        Dictionary<string, JsonElement> after)
    {
        ...
        return keys
            .Select(k => new SicknessAuditFieldChangeItem(k,
