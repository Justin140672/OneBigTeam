...
internal sealed class Endpoint(ExpireSharedCompanyDocumentHandler handler)
    : Endpoint<ExpireSharedCompanyDocumentRequest, ExpireSharedCompanyDocumentResponse>
{
    ...
    {
        if (!Guid.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier), out var expiredBy))
        {
        ...
        var result = await handler.HandleAsync(request, expiredBy, cancellationToken);
