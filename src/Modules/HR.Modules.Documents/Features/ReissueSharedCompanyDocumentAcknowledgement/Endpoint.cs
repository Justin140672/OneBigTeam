...
internal sealed class Endpoint(ReissueSharedCompanyDocumentAcknowledgementHandler handler)
    : Endpoint<ReissueSharedCompanyDocumentAcknowledgementRequest, ReissueSharedCompanyDocumentAcknowledgementResponse>
{
    ...
    {
        if (!Guid.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier), out var reissuedBy))
        {
        ...
        var result = await handler.HandleAsync(request, reissuedBy, cancellationToken);
