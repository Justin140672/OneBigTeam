...
| `specifications/runbooks/deployment-pipeline.md` | §5 (release-safety declaration, `-ExpectedSha` gate, `/health/release`), §6 (capture/restore/re-verify, migration-incompatible guard, "code only, never data" wording), §8/§9. |
...
`specifications/runbooks/deployment-pipeline.md` §6.
...
`grep -ri "redeploy --deployment\|redeploy latest\|redeploy the latest\|latest-deployment fallback" specifications/`
...
| `grep -ri "redeploy --deployment\|redeploy latest\|redeploy the latest\|latest-deployment fallback" specifications/` | ripgrep | only negative statements remain | lead developer |
