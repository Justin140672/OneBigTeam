...
- `specifications/compliance/data-protection-operations.md` ("Customer cancellation, return and deletion")
- `specifications/runbooks/backup-and-disaster-recovery.md` (§5.5 reapplication of deletion obligations)
...
| 9 | **Equality & diversity monitoring data** (voluntary special-category answers) | `employees.employee_equality_data` (answer columns AES-256-GCM encrypted) | Controller-set, but bound to the employee lifecycle. Retained only while the employee record exists; not retained independently of it. Purpose is equality monitoring, served by the anonymous aggregate report. | **No** period knob. The employee can withdraw their own record at any time (`DeleteMyEqualityData`). | **Automatic on employee/company deletion.** `ON DELETE CASCADE` FK to `employees.employees` (migration `AddEmployeeEqualityDataEmployeeForeignKey`) — physically deleting the employee, or dropping the `employees` schema at full-tenant deletion, destroys the row. No standalone purge job: the cascade is the mechanism that prevents an identifiable orphan. Soft-delete / offboarding does **not** remove it (employee row persists). Encryption keys are held outside the DB, so backups carry ciphertext only. |
