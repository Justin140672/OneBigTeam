...
procedures (`specifications/compliance/data-protection-operations.md`).
...
| Monitoring | Add a monthly manual check (or Supabase status webhook) that the most recent successful backup timestamp is < 25h old. Record in the assurance register (`specifications/compliance/data-protection-operations.md` → "Records and assurance"). |
...
clause 10 and `specifications/compliance/data-protection-operations.md` step 6, still-valid deletion
...
2. Cross-check against the **out-of-band deletion register** (`specifications/compliance` process) — the
...
- **Local drill (repeatable now):** `specifications/runbooks/restore-drill.md` +
...
| 15 | DPA §10 / Privacy Policy "Retention" (NFR-07) | Deletion instructions and legal holds "that remain applicable will be reapplied" after a restore | §5.5 steps 1–6 + `specifications/compliance/data-retention-inventory.md` | **Documented**; depends on G4 register capturing legal holds |
...
- **G4:** Out-of-band deletion obligations register (needed for DPA §10 reapplication after
  ...
  (`specifications/compliance/data-retention-inventory.md`) is the reference for which data classes carry a
  deletion obligation.
