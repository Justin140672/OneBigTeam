...
the availability & health-monitoring runbook (`specifications/runbooks/availability-and-health-monitoring.md`)
and the backup & disaster-recovery runbook (`specifications/runbooks/backup-and-disaster-recovery.md`).
...
(`specifications/runbooks/backup-and-disaster-recovery.md`) to just before the deploy **first**, then
...
3. Application rollback restores **code only, never data**. If the bad deploy ran a **contracting**
   ...
   migrated schema — restore the database using `specifications/runbooks/backup-and-disaster-recovery.md`
...
4. Record the rollback: re-run is logged via the GitHub Deployments API + job summary; add an
   incident note under `specifications/reviews/` if customer-impacting > 30 min (per the availability
   runbook §6.3).
...
in the assurance register (`specifications/compliance/data-protection-operations.md`).
