...
encrypted fields
Related: `specifications/security/sensitive-data-encryption.md` (mechanism),
`specifications/compliance/data-protection-operations.md` (DPA/operational context),
`specifications/runbooks/backup-and-disaster-recovery.md` (restore + deletion re-application)
...
- Restores: after restoring an earlier database copy, reapply all still-valid customer-deletion
  obligations (`specifications/runbooks/backup-and-disaster-recovery.md` §5.5) before returning to service.
...
6. If a key is believed compromised, treat as a security incident
   (`specifications/compliance/data-protection-operations.md` — Personal data breaches): rotate immediately
