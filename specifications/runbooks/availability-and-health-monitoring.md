...
disaster recovery runbook (`specifications/runbooks/backup-and-disaster-recovery.md`).
...
`specifications/runbooks/` follow-ups (not yet created — see open questions).
...
5. **Post-incident** — within 3 business days: written incident review (timeline, root cause,
   error-budget impact, corrective actions). File under `specifications/reviews/`.

Follow the same on-call handoff and console-access model as the backup & disaster recovery runbook
(`specifications/runbooks/backup-and-disaster-recovery.md` §on-call).
